...
export default function QRScannerPage() {
  ...
  // Real QR codes generated by the backend (see qrCode.controller.js /
  // harvest.controller.js) encode a full verification URL, e.g.
  // "https://grochain.app/verify/BATCH-2026-001" — not the bare batch ID.
  // The verify endpoint is GET /api/verify/:batchId, so a scanned URL must be
  // reduced to its last path segment before being sent, otherwise every real
  // (non-manual-sample) QR scan 404s against a malformed path.
  const extractBatchId = (raw: string): string => {
    const trimmed = raw.trim()
    try {
      const url = new URL(trimmed)
      const segments = url.pathname.split('/').filter(Boolean)
      const verifyIndex = segments.findIndex(segment => segment.toLowerCase() === 'verify')
      const candidate = verifyIndex !== -1 ? segments[verifyIndex + 1] : segments[segments.length - 1]
      if (candidate) return decodeURIComponent(candidate)
    } catch {
      // Not a URL — assume it's already a bare batch ID
    }
    return trimmed
  }
  ...
  const processScannedData = async (rawData: string) => {
    ...
    const data = extractBatchId(rawData)

    try {
      console.log('🔍 Processing scanned data:', rawData, '→ batchId:', data)
